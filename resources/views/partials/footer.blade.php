    ...
    <div id="floating-chat"
        ...
        <div
            ...
            {{-- <button onclick="toggleChat()" id="chat-fab"
                class="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-orange-500 to-orange-600 rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-110 transition-all z-50">
...
{{-- <script>
    ...
    } // Tambahkan ini di script chatbot Anda
    const input = document.getElementById('chat-input');
    const sendBtn = document.getElementById('send-btn');
    const chatBox = document.querySelector('.overflow-y-auto'); // Area pesan
    ...
    function kirimPesan() {
        ...
        tambahPesanKeChat('user', pesan);
        input.value = '';

        // 2. Kirim ke Backend
        fetch('/api/chat', { // Pastikan route ini ada di api.php
                method: 'POST',
                headers: {
            ...
            .then(res => res.json())
            .then(data => {
                tambahPesanKeChat('bot', data.reply);
            })
    ...
    // Fungsi helper tambah pesan ke UI
    function tambahPesanKeChat(sender, text) {
        const div = document.createElement('div');
        div.className = sender === 'user' ? 'flex justify-end' : 'flex justify-start';
        div.innerHTML =
            `<div class="${sender === 'user' ? 'bg-orange-600 text-white' : 'bg-white border'} px-4 py-2 rounded-2xl text-sm max-w-[80%]">${text}</div>`;
